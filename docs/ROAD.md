...
**Status / 状态:** Active — long-term goal / 生效中——远期目标（设计保留，近期不排实施期；世界线先导见 Overturn）
**Date / 日期:** 2026-09-08
...
* 定位更新（2026-09-15）：Toterra 为系列**远期目标**——六条设计线与世界观文档全部保留，近期不排实施期；世界线同域内容（litho 地层 / 气候植被 / 矿物母岩）由先行模组 **Overturn**（世界生成拟真先锋，Subterra 平台第一个真实消费方）先导验证，未来并入（详见聚合仓 `docs/2026-09-15-overturn-mod-design.md`）。/ Positioning update (2026-09-15): Toterra is the series **long-term goal** — all design lines and worldview docs are kept, no near-term implementation cycle; the world-line domain (litho strata / climate vegetation / mineral-by-rock) is first pioneered by **Overturn** (the worldgen-realism vanguard, Subterra's first real consumer) and merges back later (see aggregate-repo `docs/2026-09-15-overturn-mod-design.md`).
...
* 2026-09-15 — 系列规划调整：Toterra 调整为**远期目标**（六条设计线与世界观文档全部保留，近期不排实施期）；世界线 p.1.0–p.1.1 同域内容由先行模组 **Overturn**（世界生成拟真先锋：litho 地层 / 气候植被 / 矿物母岩）先导验证，未来并入；设计见聚合仓 `docs/2026-09-15-overturn-mod-design.md`。 / 2026-09-15 — series plan adjustment: Toterra deferred to the **long term** (all design lines and worldview docs kept, no near-term cycle); the world-line p.1.0–p.1.1 domain is first pioneered by **Overturn** (worldgen-realism vanguard: litho strata / climate vegetation / mineral-by-rock) and merges back later; design in aggregate-repo `docs/2026-09-15-overturn-mod-design.md`.
* 2026-09-10 — 思绪系统定案（p.2.5 独立横切小节）：思绪 ≠ 任务（无目标/进度/奖励/失败）；实物手记载体 + 六类页 + 抄录消耗纸墨；关联三者皆有（自动归拢 / 玩家主动拼合 / 世界反馈走零 HUD）；玩家自写待办亲手划掉；全知识收纳；分仓 Subterra `engine.thought`（纯 JDK + 确定性探针）与 Toterra 手记；td 定型配置 + td/zd 持久化，藏录互操作；设计文档 `docs/2026-09-10-thoughts-system-design.md`。 / Thoughts system decided (p.2.5 standalone cross-cutting section): thoughts ≠ quests; a world-borne journal item with six page kinds; three connection mechanics (auto grouping / player matching / zero-HUD world feedback); hand-crossed to-dos; all-knowledge collection; Subterra `engine.thought` (pure JDK + deterministic probe) + Toterra journal split; td-shaped config + td/zd persistence; Ledger interop; design doc `docs/2026-09-10-thoughts-system-design.md`.
